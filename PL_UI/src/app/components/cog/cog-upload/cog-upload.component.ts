...
import { COGUploadResult } from 'src/app/shared/models/cog-upload-result.model';
...
import { SaleType } from 'src/app/shared/models/sale-type.model';
...
@Component({
  selector: 'app-cog-upload',
  templateUrl: './cog-upload.component.html',
  styleUrls: ['./cog-upload.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class COGUploadComponent implements OnInit {
  ...
  productSubCategoryMultiSelectSettings = {};
  productTypeMultiSelectSettings = {};
  ...
  initializeFormControl() {
    ...
    this.productSubCategoryMultiSelectSettings = {
      singleSelection: false,
    ...
    this.productTypeMultiSelectSettings = {
      singleSelection: false,
  ...
  createUploadForm(){
    this.uploadForm = this._fBuilder.group({
      SalesTypeIdUpload: new FormControl(null, [Validators.required]),
      SalesSubTypeUpload: new FormControl('Monthly', [Validators.required]),
    });
  }
  onSaleTypeChange(){
    console.log('Sale Type changed:', this.saleTypeList);    
  }
  backButton(){
    this.reload();
   }
  ...
  onSearch() {
    ...
    var command = {
      ...
      SalesTypeId: this.f.SalesTypeId.value,
      SalesSubType: this.f.SalesSubType.value,
  ...
  loaddata(command){
    this.dataList = this.cogservice.getuploadCOGData(command);
  ...
  loadCustomer() {
    this.masterService.getCustomersByUserCountryIds(null)
      ...
      .subscribe({
        next: (x) => (
          this.getCustomer = x.map(el => { return { 
  ...
  onSubmit() {
    ...
    formData.append("FolderPath", "cog-files");
    formData.append("File", this.files[0]);
    formData.append("FileTypeId", "1");
    formData.append("SaleTypeId", formValue.SalesTypeIdUpload);
    formData.append("SaleSubType", formValue.SalesSubTypeUpload);
    this.clearAllErrors();
    this.cogservice.uploadCOG(formData)
      .pipe()
      .subscribe({
  ...
  handleSuccess(data: any) {
    ...
    if (data && data.isSuccess) {
      const responseMessages: COGUploadResult[] = data.value;
      const successData = responseMessages.find(c=> c.responseCode === '200');

      if(successData){
        this.clearForm();
        Swal.fire('Success', "COGploaded successfully with " + successData.responseMessage, 'success');
        this.reload();
      }
      else{
        ...
        if(serverErrorData){
          ...
          this.showError = true;
          Swal.fire('Error', "Problem occured while uploading COG Entries, kidly review error message", 'error');
